...
class KecamatanKecModel extends Model
{
    ...
    public function getKecTotal($kode_kec)
    {
        ...
        $query3  = $db->query("select * , b.nama, c.deskripsi, a.alamat,f.jumgap,f.kode_bp3k,g.jumkep,d.jumpok,e.jumthl,h.jumpns,i.unit_kerja
                                ...
                                left join (select kode_kec, count(id_poktan) as jumpok from tb_poktan GROUP BY kode_kec)d on a.kode_bp3k=d.kode_kec
                                ...
                                left join(select kode_bp3k,count(id_kep) as jumkep from tb_kep GROUP BY kode_bp3k )g on a.kecamatan=g.kode_bp3k
                                left join (select tempat_tugas,count(id) as jumpns from tbldasar GROUP BY tempat_tugas) h on a.id=h.tempat_tugas and kode_kab='4' 
